// "use client";

// // import { auth } from "@/auth";
// // import FileUpload from "@/src/components/FileUpload";
// // import { ragChat } from "@/src/lib/rag-chat";
// import React, { useState } from "react";
// import { useChat } from "ai/react";
// import { TextIcon } from "@radix-ui/react-icons";
// import { Label } from "@/src/components/ui/label";
// import { Input } from "@/src/components/ui/input";
// import { Button } from "@/src/components/ui/button";
// import { Textarea } from "@/src/components/ui/textarea";

// const page = () => {
//   // const session = await auth();

//   const { messages, input, handleInputChange, handleSubmit, setMessages } =
//     useChat({
//       api: "/api/chat",
//     });

//   // const [fileUrl, setFileUrl] = useState<string>("");
//   const [fileUrl, setFileUrl] = useState<File | null>(null);
//   const [uploadedPdf, setUploadedPdf] = useState<File>();
//   const [isFileUploaded, setIsFileUploaded] = useState<boolean>(false);
//   const [msg, setMsg] = useState<string>("");

//   async function handleUpload() {
//     if (fileUrl && !isFileUploaded) {
//       setMsg("Uploading...");
//       const response = await fetch("/api/pdf-extractor", {
//         method: "POST",
//         body: JSON.stringify({ data: uploadedPdf }),
//         headers: {
//           "Content-Type": "application/json",
//           "req-type": "fileUpload",
//         },
//       });

//       console.log("Res", response.status);

//       if (response.ok) {
//         setIsFileUploaded(true);
//         setMsg("File uploaded successfully");
//         setMessages([
//           {
//             id: "-1",
//             role: "system",
//             content: "Hi, how can I help you today?",
//           },
//         ]);
//       } else {
//         setMsg("Error while uploading file");
//         console.log("Error:", response.status);
//       }
//     } else {
//       console.log("No file selected");
//     }
//   }

//   const handleFileChange = async (
//     event: React.ChangeEvent<HTMLInputElement>
//   ) => {
//     const files = event.target.files;

//     if (files && files.length > 0) {
//       const selectedPdf = files[0];
//       console.log(selectedPdf);

//       // setFileUrl(URL.createObjectURL(selectedPdf));
//       setFileUrl(selectedPdf);

//       console.log(selectedPdf);

//       const formData = new FormData();
//       formData.append("file", selectedPdf);
//       console.log("keoe", formData);

//       const response = await fetch("/api/uploads", {
//         method: "POST",
//         body: formData,
//       });
//       console.log("Res filechange", response.headers);

//       if (response.ok) {
//         const data = await response.json();
//         setUploadedPdf(data.filePath);
//       } else {
//         console.log("Error:", response.status);
//       }
//     }
//   };

//   const handleKeyPress = (event: React.KeyboardEvent<HTMLTextAreaElement>) => {
//     if (event.key === "Enter") {
//       event.preventDefault();
//       handleSubmit(event as any);
//     }
//   };

//   // await ragChat.context.add({

//   // })

//   return (
//     <div className="flex flex-col items-center justify-center h-screen">
//       <div className="grid w-full max-w-3xl px-4 gap-4">
//         <div className="flex items-center gap-4">
//           <div className="flex items-center gap-2">
//             <TextIcon className="w-8 h-8" />
//             <h1 className="text-2xl font-bold tracking-tighter">PDF Chat</h1>
//           </div>
//         </div>
//         <div className="grid gap-4">
//           <div className="grid gap-1.5">
//             <Label className="leading-none" htmlFor="upload">
//               Select a PDF to upload
//             </Label>
//             <div className="flex items-center gap-4">
//               <Input
//                 accept=".pdf"
//                 id="upload"
//                 type="file"
//                 name="file"
//                 onChange={handleFileChange}
//                 className="max"
//               />
//               <Button
//                 className="upload-button"
//                 disabled={!fileUrl}
//                 type="button"
//                 onClick={handleUpload}
//               >
//                 Upload
//               </Button>
//             </div>
//             {msg && <span>{msg}</span>}
//           </div>
//         </div>

//         <div className="grid gap-4">
//           <div className="grid gap-1.5">
//             <Label className="leading-none" htmlFor="conversation">
//               Conversation
//             </Label>
//             <div className="border p-4 rounded-lg h-48 overflow-y-auto">
//               {messages.map((message, index) => (
//                 <div
//                   key={index}
//                   className={`chat-bubble ${
//                     message.role === "user" ? "user-message" : "system-message"
//                   }`}
//                 >
//                   <div className="chat-content">
//                     <p className="message">{message.content}</p>
//                   </div>
//                 </div>
//               ))}
//             </div>
//           </div>
//           <div className="grid gap-1.5">
//             <Textarea
//               id="message"
//               onChange={handleInputChange}
//               onKeyPress={handleKeyPress}
//               placeholder="Type your message"
//               value={input}
//               disabled={!isFileUploaded}
//             />
//           </div>
//           <div className="flex justify-center">
//             <form onSubmit={handleSubmit}>
//               <Button type="submit" disabled={!isFileUploaded}>
//                 Send
//               </Button>
//             </form>
//           </div>
//         </div>
//       </div>
//     </div>
//   );
// };

// export default page;

"use client";

import { useDropzone } from "react-dropzone";
import React, { useState } from "react";
import { useChat } from "ai/react";
import { TextIcon } from "@radix-ui/react-icons";
import { Label } from "@/src/components/ui/label";
import { Button } from "@/src/components/ui/button";
import { Textarea } from "@/src/components/ui/textarea";

const Page: React.FC = () => {
  const { messages, input, handleInputChange, handleSubmit, setMessages } =
    useChat({
      api: "/api/chat",
    });

  const [fileUrl, setFileUrl] = useState<File | null>(null);
  const [uploadedPdf, setUploadedPdf] = useState<File>();
  const [isFileUploaded, setIsFileUploaded] = useState<boolean>(false);
  const [msg, setMsg] = useState<string>("");

  // Dropzone configuration
  const onDrop = (acceptedFiles: File[]) => {
    if (acceptedFiles.length > 0) {
      const selectedPdf = acceptedFiles[0];
      console.log(selectedPdf);

      setFileUrl(selectedPdf); // Save the selected PDF file
      handleFileChange(selectedPdf); // Trigger file upload function
    }
  };

  const { getRootProps, getInputProps } = useDropzone({
    onDrop,
    // accept: { "application/pdf": [".pdf"] }, // Accept only PDF files
    multiple: false,
  });

  const handleFileChange = async (selectedPdf: File) => {

    console.log("Selected file:", selectedPdf);

    const formData = new FormData();
    formData.append("file", selectedPdf);
    console.log(formData);

    const response = await fetch("/api/uploads", {
      method: "POST",
      body: formData,
    });

    if (response.ok) {
      const data = await response.json();
      setUploadedPdf(data.filePath);
      console.log("File uploaded successfully:", data);
    } else {
      console.log("Error uploading file:", response.status);
    }
  };

  async function handleUpload() {
    if (fileUrl && !isFileUploaded) {
      setMsg("Uploading...");
      const response = await fetch("/api/pdf-extractor", {
        method: "POST",
        body: JSON.stringify({ data: uploadedPdf }),
        headers: {
          "Content-Type": "application/json",
          "req-type": "fileUpload",
        },
      });

      if (response.ok) {
        setIsFileUploaded(true);
        setMsg("File uploaded successfully");
        setMessages([
          {
            id: "-1",
            role: "system",
            content: "Hi, how can I help you today?",
          },
        ]);
      } else {
        setMsg("Error while uploading file");
        console.log("Error:", response.status);
      }
    } else {
      console.log("No file selected");
    }
  }

  const handleKeyPress = (event: React.KeyboardEvent<HTMLTextAreaElement>) => {
    if (event.key === "Enter") {
      event.preventDefault();
      handleSubmit(event as any);
    }
  };

  return (
    <div className="flex flex-col items-center justify-center h-screen">
      <div className="grid w-full max-w-3xl px-4 gap-4">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <TextIcon className="w-8 h-8" />
            <h1 className="text-2xl font-bold tracking-tighter">PDF Chat</h1>
          </div>
        </div>
        <div className="grid gap-4">
          <div className="grid gap-1.5">
            <Label className="leading-none">Select a PDF to upload</Label>
            <div
              {...getRootProps({
                className:
                  "dropzone bg-gray-900 p-6 text-sm border-dashed border-2 border-primary text-center",
              })}
            >
              <input {...getInputProps()} />
              <p>Drag & drop a file here, or click to select a file</p>
            </div>

            {fileUrl && (
              <div className="mt-4">
                <h4>Selected File:</h4>
                <p className="my-2">{fileUrl.name}</p>
                <Button
                  className="upload-button"
                  onClick={handleUpload}
                  disabled={!fileUrl || isFileUploaded}
                >
                  {isFileUploaded ? "File Uploaded" : "Upload File"}
                </Button>
              </div>
            )}

            {msg && <span>{msg}</span>}
          </div>
        </div>

        <div className="grid gap-4">
          <div className="grid gap-1.5">
            <Label className="leading-none" htmlFor="conversation">
              Conversation
            </Label>
            <div className="border p-4 rounded-lg h-48 overflow-y-auto">
              {messages.map((message, index) => (
                <div
                  key={index}
                  className={`chat-bubble ${
                    message.role === "user" ? "user-message" : "system-message"
                  }`}
                >
                  <div className="chat-content">
                    <p className="message">{message.content}</p>
                  </div>
                </div>
              ))}
            </div>
          </div>
          <div className="grid gap-1.5">
            <Textarea
              id="message"
              onChange={handleInputChange}
              onKeyPress={handleKeyPress}
              placeholder="Type your message"
              value={input}
              disabled={!isFileUploaded}
            />
          </div>
          <div className="flex justify-center">
            <form onSubmit={handleSubmit}>
              <Button type="submit" disabled={!isFileUploaded}>
                Send
              </Button>
            </form>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Page;
